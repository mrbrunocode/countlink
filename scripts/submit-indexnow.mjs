...
async function main() {
  ...
  if (urls.length === 0) {
    console.error("No URLs found in sitemap.xml — run scripts/build-timer-pages.mjs first.");
    process.exit(1);
  }
  const host = new URL(urls[0]).host;
  if (host.endsWith(".example")) {
    console.error(`sitemap.xml still points at a placeholder domain (${host}).`);
    console.error("Update SITE_URL in scripts/build-timer-pages.mjs to the real domain, re-run it, then re-run this script.");
    process.exit(1);
  }
